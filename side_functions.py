...
def round_me(x, LoD, elem):
    ...
    if x < LoD[elem]: return '< LoD'
    else:
        if x >= 100: return float(Decimal(x).quantize(Decimal('1')))
        else:
            if x >= 10: return float(Decimal(x).quantize(Decimal('0.1')))
            else:
                if x >= 1: return float(Decimal(x).quantize(Decimal('0.01')))
                else:

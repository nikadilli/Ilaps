...
from layouts.calibration import Calibration
...
class MainWindow(QMainWindow):
    ...
    def init_gui(self):
        ...
        self.Stack.addWidget(self.calibration.get_page())
        ...
        imaging.triggered.connect(lambda: self.change_layout(4))

        table = QAction(QtGui.QIcon('./imgs/table.png'), 'Show data', self)
        table.setShortcut('Ctrl+T')
        table.triggered.connect(lambda: self.change_layout(5))

        error = QAction(QtGui.QIcon('./imgs/error.png'), 'Show error log', self)
        error.setShortcut('Ctrl+E')
        error.triggered.connect(lambda: self.change_layout(6))

        self.toolbar = self.addToolBar('Exit')
        self.toolbar.addAction(home)
        self.toolbar.addAction(data)
        self.toolbar.addAction(calibration)
